...
#small_trans="3 5"
#small_conv="5"
#save_path="runs/freq_upsampling/voc2012/unet"
			...
			do
				...
				do
					...
					small_conv="0"
		                    ...
		                    folder="${save_path}/${net}_trans_kernel_${trans}_small_trans_${st}_convnext_backbone_${use_convx}_backbone_kernel_${bk}_small_conv_${small_conv}/val/ss"
							var=$(ls ${path}| sort -V | tail -n 1)
							model_location="${path}/${var}"
							
		                    env
				    sbatch -p gpu -t 23:59:59 --gres=gpu:2 --ntasks=1 --cpus-per-task=8 --mem=50G job_maker/unet_voc_acc.sh $net $epoch $lr $path $folder $optimizer $criteria $trans $bk $st $small_conv $use_convx $model_location
				done
			    done
			done
		    done
