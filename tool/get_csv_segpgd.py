...
path_dir = "/work/ws-tmp/sa058646-segment/semseg/runs/new_exp_slak/voc2012/unet_extra/segpgd_attack"
csv_file = "/work/ws-tmp/sa058646-segment/semseg/cvpr_rebuttal_convnext_segpgd.csv"


path_logs = "/work/ws-tmp/sa058646-segment/semseg/runs/new_exp_slak/voc2012/unet_extra/segpgd_attack/**/val/ss/**/log.log"
    ...
    with open(path) as log:
        for line in log:
            if 'Eval result: mIoU/mAcc/allAcc' in line:
                ...
                path = Path(path)
                ep = path.parent.absolute().name.replace('iterations_', '')
...
csv_dict = {"transposed conv kernel": trans_cov_kernel, "small parallel trans kernel": trans_small, "use ConNeXt backbone": use_convnext,
            ...
            "Attack Iterations": eps, "mIoU":mIoUs, "mAcc": mAccs, "allAcc": allAccs}
