...
deeplab_dir = os.path.dirname("/work/ws-tmp/sa058646-segment/DeepLabV3Plus-Pytorch")
sys.path.append(deeplab_dir)
import network
...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='/work/ws-tmp/sa058646-segment/PSPNet/config/voc2012/voc2012_pspnet50.yaml', help='config file')
...
def main():
    ...
    index_end = args.index_end if args.index_end-index_start < 1000 else len(test_data.data_list)
    ...
    with torch.enable_grad():
        ...
        for t in range(iterations):
            ...
            if attack == 'segpgd':
                ...
                one_hot_target = torch.nn.functional.one_hot(target, num_classes=21).permute(0,3,1,2)
                eps=10**-8
