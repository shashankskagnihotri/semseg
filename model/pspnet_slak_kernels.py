...
class Eleven(nn.Module):
    def __init__(self, psp_kernel, fea_dim):
        ...
        #fea_dim = 2048
        self.layer1=nn.Conv2d(fea_dim, 512, kernel_size=psp_kernel, stride=1, padding=4, bias=False)
        self.layer2=nn.Conv2d(fea_dim, 512, kernel_size=3, stride=1, padding=0, bias=False)
    def forward(self, x):
        #layer1_out = layer1(x)
        #layer2_out = layer2(x)
...
class PSPNet(nn.Module):
    def __init__(self, layers=50, bins=(1, 2, 3, 6), dropout=0.1, classes=2, zoom_factor=8, use_ppm=True, criterion=nn.CrossEntropyLoss(ignore_index=255), pretrained=True, psp_kernel=None):
        ...
        if psp_kernel !=11:
            self.large_kernels = nn.Conv2d(fea_dim, 512, kernel_size=psp_kernel, stride=1, padding=0, bias=False)
        else:
